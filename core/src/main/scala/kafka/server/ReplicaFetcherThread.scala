 ...
 * @param name // 线程名称
 * @param fetcherId
 *                  Follower 拉取的线程 Id，也就是线程的编号。单台 Broker 上，允许存在多个 ReplicaFetcherThread 线程。
 *                  Broker 端参数 num.replica.fetchers，决定了 Kafka 到底创建多少个 Follower 拉取线程。
 * @param sourceBroker // 数据源Broker地址
 *                     源 Broker 节点信息。源 Broker 是指此线程要从哪个 Broker 上读取数据。
 * @param brokerConfig 封装了 Broker 端所有的参数信息
 * @param failedPartitions // 处理过程中出现失败的分区
 *                         线程处理过程报错的分区集合。
 * @param replicaMgr 副本管理器。该线程类通过副本管理器来获取分区对象、副本对象以及它们下面的日志对象。
 * @param metrics
 * @param time
 * @param quota 用做限流。限流属于高阶用法，
 * @param leaderEndpointBlockingSend 这是用于实现同步发送请求的类。所谓的同步发送，是指该线程使用它给指定 Broker 发送请求，
...
class ReplicaFetcherThread(name: String,
  ...
  override def processPartitionData(topicPartition: TopicPartition,
    ...
    // 要读取的起始位移值如果不是本地日志LEO值则视为异常情况 if (fetchOffset != log.logEndOffset)
