...
class MetadataCache(brokerId: Int) extends Logging {
  ...
  // 接收主题分区和 ListenerName,以获取指定监听器类型下该主题分区所有副本的 Broker 节点对象，并按照 Broker ID 进行分组。
  ...
  /**
   *
   * @param partitionStates 这是一个 Map 类型。Key 是主题名称，Value 又是一个 Map 类型，
   *                        其 Key 是分区号，Value 是一个 UpdateMetadataPartitionState 类型的字段。
   * @param controllerId Controller 所在 Broker 的 ID。
   * @param aliveBrokers 当前集群中所有存活着的 Broker 对象列表。
   * @param aliveNodes 这也是一个 Map 的 Map 类型。其 Key 是 Broker ID 序号，Value 是 Map 类型，
   *                   其 Key 是 ListenerName，即 Broker 监听器类型，而 Value 是 Broker 节点对象。
   */
