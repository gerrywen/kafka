...
class KafkaRequestHandler(id: Int,
  ...
  def run(): Unit = {
    ...
    while (!stopped) {
      ...
      //我们使用一个计量表来表示线程池的聚合空闲百分比。
      //因为meter的计算方式是total_recorded_value / time_window和
      // time_window与线程数无关，每个线程记录为空闲
      //时间应该以# threads来折扣。
 ...
 *                 和 KafkaRequestHandler 中的一样，保存 Broker 的序号。
 * @param requestChannel SocketServer组件下的RequestChannel对象
 *                       SocketServer 的请求处理通道，它下辖的请求队列为所有 I/O 线程所共享。
 *                       requestChannel 字段也是 KafkaRequestHandler 类的一个重要属性。
 * @param apis KafkaApis类，实际请求处理逻辑类
 *             KafkaApis 实例，执行实际的请求处理逻辑。它同时也是 KafkaRequestHandler 类的一个重要属性。
 * @param time
 * @param numThreads I/O线程池初始大小
 *                   线程池中的初始线程数量。它是 Broker 端参数 num.io.threads 的值。
 *                   目前，Kafka 支持动态修改 I/O 线程池的大小，因此，这里的 numThreads 是初始线程数，
 *                   调整后的 I/O 线程池的实际大小可以和 numThreads 不一致。
...
class KafkaRequestHandlerPool(val brokerId: Int,
   ...
   * @param newSize  把标识线程数量的变量 threadPoolSize 的值调整为目标值
   */
  def resizeThreadPool(newSize: Int): Unit = synchronized {
    val currentSize = threadPoolSize.get
    info(s"Resizing request handler thread pool size from $currentSize to $newSize")
    if (newSize > currentSize) {
      // 目标数量比当前数量大. createHandler 方法将线程数补齐到目标值 newSize
