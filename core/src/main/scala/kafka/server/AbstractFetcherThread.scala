 ...
 *
 * @param name // 线程名称
 * @param clientId // Client Id，用于日志输出
 * @param sourceBroker // 数据源Broker地址
 *                     源 Broker 节点信息。源 Broker 是指此线程要从哪个 Broker 上读取数据。
 * @param failedPartitions // 处理过程中出现失败的分区
 *                         线程处理过程报错的分区集合。
 * @param fetchBackOffMs // 获取操作重试间隔
 *                       当获取分区数据出错后的等待重试间隔，默认是 Broker 端参数 replica.fetch.backoff.ms 值。
 * @param isInterruptible // 线程是否允许被中断
 * @param brokerTopicStats // Broker端主题监控指标
 *                         Broker 端主题的各类监控指标，常见的有 MessagesInPerSec、BytesInPerSec 等。
 *
 * 比较重要的是 sourceBroker，因为它决定 Follower 副本从哪个 Broker 拉取数据，也就是 Leader 副本所在的 Broker 是哪台。
 */
abstract class AbstractFetcherThread(name: String,
  ...
  /**
   *
   * @param partitionMap // 一组要读取的分区列表
   *                     // 分区是否可读取取决于PartitionFetchState中的状态
   * @return // 封装FetchRequest.Builder对象
   */
