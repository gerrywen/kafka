...
class ControllerContext {
  ...
  var offlinePartitionCount = 0 // 离线分区计数器。该字段统计集群中所有离线或处于不可用状态的主题分区数量。所谓的不可用状态，就是我最开始举的例子中“Leader=-1”的情况。
  var preferredReplicaImbalanceCount = 0 // 首选副本不平衡计数器
  ...
  // getter
  // 从中移除关闭中的 Broker 序号，剩下的自然就是处于运行中的 Broker 序号列表
  ...
  // 根据给定主题分区的当前状态和目标状态，来判断该分区是否是离线状态的分区. 更新offlinePartitionCount元数据
